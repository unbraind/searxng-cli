...
run "searxng instance source-status --json >/tmp/searxng-e2e-source-status.json"
...
if command -v jq >/dev/null 2>&1; then
  ...
  jq -e '.data as $data | .format == "instance-source-status" and $data.status == "current" and ($data.upstream.commit | startswith($data.live.commit))' /tmp/searxng-e2e-source-status.json >/dev/null
  ...
  "$NODE_BIN" -e 'const j=JSON.parse(require("fs").readFileSync("/tmp/searxng-e2e-source-status.json","utf8")); if(j.format!=="instance-source-status"||j.data?.status!=="current"||!j.data?.upstream?.commit?.startsWith(j.data?.live?.commit)) process.exit(1);'
